...
internal static class HealthCheckInstaller
{
    ...
    public static IServiceCollection AddAbsHealthCheckInstaller(this IServiceCollection services, IConfiguration configuration)
    {
        if (!String.IsNullOrEmpty(configuration.GetSection("ASB:TopicDriver")!.Value))
        {
            services.AddHealthChecks().AddAzureServiceBusTopic(configuration.GetSection("ASB:EndPoint")!.Value,
                configuration.GetSection("ASB:TopicDriver")!.Value,
