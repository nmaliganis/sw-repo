...
using dottrack.auth.Integration.tests.V1.Account;
...
namespace dottrack.auth.Integration.tests.V1.Accounts
{
    ...
    public class AccountsScenarios
      : AccountsScenariosBase, IDisposable
    {

        [Theory (Skip = "Not ready yet")]
        [ClassData(typeof(CreateAccountsTestData))]
        public async Task CreatingAccount_ShouldReturnOKResponse(UserForRegistrationUiModel createParams)
        {
            this.Startup();
            ...
            var postResponse = await client.PostAsync(Post.PostAccountRegisterAsync(), requestCreationContent);
            if (postResponse.StatusCode.Equals(HttpStatusCode.BadRequest))
            {
                ...
                while (reasonOfBadRequest.Equals("ERROR_Department_ALREADY_EXISTS")) // IT SHOULD BE REFACTORED ACCORDING TO THE CORRECT MESSAGE -- see createExistingDepartment TEST
                {
                    ...
                    postResponse = await client.PostAsync(Post.PostAccountRegisterAsync(), requestCreationContent);
